...
$(function() {
    WJG = {
        flash: function(obj) {
            $(obj.li).css("position", "relative");
            $(obj.li).find(obj.point).css({
                "position": "absolute"
            });
            $(obj.li).each(function() {
                $(this).find(obj.point).each(function() {
                    $(this).css({
                        "left": ($(this).index() % obj.cospan) * (obj.b_wid + 16) + 29 + "px",
                        "top": Math.floor($(this).index() / obj.cospan) * (obj.b_hei + 13) + 17 + "px",
                        "background": "url(http://img.chemayi.com/newindex/new_small.png) no-repeat 0px " + (0 - parseInt($(this).index()) * obj.b_hei) + "px"
                    })
                })
            });
            $(obj.li).find(obj.point).hover(function() {
                $(this).css({
                    "left": parseInt($(this).css("left")) - 9 + "px",
                    "top": parseInt($(this).css("top")) - 9 + "px",
                    "width": obj.m_wid + "px",
                    "height": obj.m_hei + "px",
                    "z-index": 9
                });
                $(this).css({
                    "background": "url(http://img.chemayi.com/newindex/new_big.png) no-repeat 0px " + (0 - parseInt($(this).index()) * obj.m_hei) + "px"
                });
                $(this).siblings(".service_ul_cover").show()
            }, function() {
                $(this).css({
                    "left": ($(this).index() % obj.cospan) * (obj.b_wid + 16) + 29 + "px",
                    "top": Math.floor($(this).index() / obj.cospan) * (obj.b_hei + 13) + 17 + "px",
                ...
                $(this).siblings(".service_ul_cover").hide()
            })
        }
    };
    WJG.flash({
...
var CMY = {
    AddNewAdver: function() {
        var d = new Date();
        var y = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate() + "<br>";
        var t = d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
        var trobj = "<tr class='movetr thenewadver order_even' index=0>" + "	<td class='biz_mes_box' >" + "	</td>" + "	<td class='biz_mes_state'>" + "		<b class='unsave'>未保存</b>" + "	</td>" + "	<td class='biz_mes_txtarea'>" + "	 <textarea class='bix_mes_txtareabox' value=''></textarea>" + "	</td>" + "	<td class='biz_modify_time tc'>" + y + t + "</td>" + "	<td class='biz_project_del tc col_a333'>" + "		<a class='lbt_btn lbt_white'  onclick='addnewAds();' href='javascript:void(0)'>" + "			<span class='l-btn-left'>" + "			<span class='l-btn-text'>保存</span>" + "			</span>" + "		</a>" + "		<a class='lbt_btn lbt_white ' href='javascript:void(0)'>" + "			<span class='l-btn-left'>" + "			<span class='l-btn-text'>删除</span>" + "			</span>" + "		</a>" + "	</td>" + "</tr>";
        $("table.biz_mes_list").find("tr").eq(0).after(trobj)
    },
    SaveNewAdver: function() {
        return addAds()
    },
    notice: function(str, bool) {
        var img = bool == true ? "notice_pass" : bool == false ? "notice_error" : "notice_tip";
        var color = bool == true ? "#427400" : bool == false ? "#d31312" : "#ff6a00";
        var speed = bool == true ? 1000 : 1500;
        var bgcover = "<div class='cmy_cover' style='width:100%;position:absolute;filter:alpha(opacity=70);-moz-opacity:0.7;opacity:0.7;z-index:99998;background:#000; top:0px;left:0px;'></div>";
        var div = " <div class='opacity_border' style='filter:alpha(opacity=30);-moz-opacity:0.3;opacity:0.3;background:#000;height:82px; position:absolute;'></div>" + " <div class='cmy_math_notice' style='position:absolute;font-size:14px;height:62px;line-height:62px;overflow:hidden; background:url(http://img.cmy4.com/common/" + img + ".jpg) no-repeat 15px center #fff; color:" + color + ";padding:0px 66px;'>" + str + "</div>";
        $("body").append(bgcover + div);
        $(".cmy_cover").css({
            "height": $(document).height() + "px"
        });
        $(".cmy_math_notice").css({
            "z-index": "100000",
            "left": ($("body").width() - $(".cmy_math_notice").width() - 10) / 2 + "px",
            "top": ($(window).height() - $(".cmy_math_notice").height() - 50) / 2 + $(window).scrollTop() + "px"
        });
        $(".opacity_border").css({
            "z-index": "99999",
            "left": $(".cmy_math_notice").offset().left - 10 + "px",
            "top": $(".cmy_math_notice").offset().top - 10 + "px",
            "width": $(".cmy_math_notice").outerWidth() + 20 + "px"
        });
        setTimeout(function() {
            $(".cmy_math_notice").fadeOut(300);
            $(".cmy_cover").fadeOut(300);
            $(".opacity_border").fadeOut(300);
            setTimeout(function() {
                $(".cmy_math_notice").remove();
                $(".cmy_cover").remove();
                $(".opacity_border").remove()
            }, 300)
        }, speed)
    },
    adverAddCheckBox: function(id) {
        var len = $(id).parents(".biz_mes_list").find(".mescheckbox").length;
        var leng = $(id).parents(".biz_mes_list").find(".mescheckbox:checked").length;
        if (len === leng) {
            $(id).parents(".biz_mes_list").find(".biz_sel_all").attr("checked", "checked")
        } else {
            $(id).parents(".biz_mes_list").find(".biz_sel_all").attr("checked", false)
        }
    },
    ShowDialog: function(id) {
        $(".cmy_layer_bg").show();
        $(id).show();
        var w = ($("body").width() - $(id).outerWidth()) / 2;
        var s_top = 0;
        var c_top = document.documentElement.clientHeight;
        var h = $(document).scrollTop();
        $(".cmy_layer_bg").css("height", $("body").height() + "px");
        $(id).css({
            "left": w + "px",
            "top": h + "px"
        })
    },
    ShowDialog2: function(id) {
        $(".cmy_layer_bg").show();
        $(id).show();
        var c_top = document.documentElement.clientHeight;
        $(".cmy_layer_bg").css("height", $('body').height() + "px")
    ...
    AddPeijian: function(id) {
        var obj = $(id).parents(".cmy_panel").find(".cmy_panel_row").eq(1).clone();
        obj.find("input").attr("value", "");
        obj.children().eq(4).html("元&nbsp;&nbsp;" + "<a onclick='javascript:deleteProw(this);' style='cursor:pointer; color:#1f7e88;'>删除</a>");
        $(id).parents(".cmy_panel_row").before(obj)
    },
    SendOrder: function() {
        if (send_order()) {
            CMY.HideDialog();
            CMY.ShowDialog(".order_sendsuccess")
        }
    },
    DesignSelect: function(id) {
        var p = id.parents(".cmy_select");
        var obj = p.siblings(".cmy_select_list");
        obj.css({
            "width": p.width() + "px",
            "left": p.offset().left - id.parents(".cmy_dialog").offset().left - 1 + "px",
            "top": p.offset().top - id.parents(".cmy_dialog").offset().top + p.height() + "px"
        });
        obj.find("li").css({
            "height": p.height() + 2 + "px",
            "line-height": p.height() + 2 + "px"
        });
        ...
        obj.find("li").click(function() {
            obj.find("li").removeClass("choose");
            $(this).addClass("choose");
            p.find("input.cmy_select_box").attr("value", $(this).html());
            obj.hide()
        });
        obj.toggle()
    },
    FlashScroll: function(config) {
        $(config.leftbtn).bind('click', function() {
            if (parseInt($(config.ulname).attr("index")) == 0) {
                $(config.ulname).attr("index", $(config.ulname).find("li").length - 1 - config.keepnum)
            } else {
                $(config.ulname).attr("index", parseInt($(config.ulname).attr("index")) - 1)
            }
            $(config.ulname).stop().animate({
                "left": 0 - parseInt($(config.ulname).attr("index")) * config.step + "px"
            }, config.speed)
        });
        $(config.rightbtn).bind('click', function() {
            if (parseInt($(config.ulname).attr("index")) == $(config.ulname).find("li").length - 1 - config.keepnum) {
                $(config.ulname).attr("index", 0)
            } else {
                $(config.ulname).attr("index", parseInt($(config.ulname).attr("index")) + 1)
            }
            $(config.ulname).stop().animate({
    ...
    Phyfinish: true,
    Deg: 0,
    AutoCheckFinish: false,
    AfterCheckFinish: false,
    PhySpeed: 300,
    PhyCount: function(classicIndex) {
        var arr = [4, 6, 10, 11, 13];
        var bool = false;
    ...
    PhyAjax: function() {
        ...
        for (var i = 0; i < jsonReturn.length; i++) {
            ...
            for (var j = 0; j < classic.length; j++) {
                ...
                CMY.PhySpId.push(classic[j]["spid"])
            }
        }
        CMY.PhyAnimate(0, 0);
    ...
    MathScore: function(proIndex, classicIndex, score) {
        ...
        if (score == 0) {
            ...
            CMY.PhyTimerScore = setInterval(function() {
                if (s == _end) {
                    clearInterval(CMY.PhyTimerScore);
                    CMY.PhyTimerScore = null;
                    if (CMY.PhyCount(classicIndex)) {
                        proIndex++
                    }
                    classicIndex++;
    ...
    ShowProgress: function(proIndex, classicIndex, score) {
        ...
        $(".progress_bar_line").stop().animate({
            "width": wid + "px"
        }, 280);
    ...
    ShowResultText: function() {
        ...
        if (score > 80) {
            $('.baoyang_box').find('.helpinfo').text(CMY.CarInfo[3]);
            var car_model = $('.new_helper_content_info input[name=car_model]').val();
            var lookulr = "/application/downsave/car_model-" + car_model + ".html";
            var downsaveurl = "/application/downsave/car_model-" + car_model + "-down-1.html";
            $('.baoyang_box').children('.baoyang_main').find('img').attr("src", lookulr);
            $('#downpic').attr("href", downsaveurl);
            $('.baoyang_box').show()
        } else {
            var str = "";
            str += "<div class='layer_result_info'>" + "<div class='physical_short'>" + "整车状况：<span class='col_cd0200'>健康</span> 目前需要：<span class='col_cd0200'>" + CMY.CarInfo[1] + "</span><br />" + CMY.CarInfo[2] + "</div>" + "<div class='physical_math'>" + CMY.CarInfo[3] + "</div>" + "<a class='makeorder'>立即生成保养清单</a>" + "</div>";
            $(".the_step_two").append(str);
            $(".layer_result_info").css("left", ($(".the_step_two").outerWidth() - $(".layer_result_info").outerWidth()) / 2 + $(".the_step_two").offset().left + "px");
            $(".layer_result_info").css({
                "top": ($(window).height() - $(".layer_result_info").height()) / 2,
                "position": "fixed"
            });
    ...
    ShowPhyResultInfo: function(proIndex, classicIndex) {
        var score = CMY.PhyStatus[classicIndex];
        var div = "<div style='" + (CMY.PhyProJy[classicIndex].indexOf("无该数据") >= 0 ? "display:none;" : "") + "' class='phy_result_info_row  " + (checkifnodata(proIndex, classicIndex) ? "bottomline" : "") + "' id=" + CMY.PhyCatid[classicIndex] + " >" + "<span class='test_pro'>" + (checkifnodata(proIndex, classicIndex) ? "<" + CMY.PhyProArr[proIndex] + ">" : "") + "</span>" + "<span class='replace_pro'>" + "<span class='replace_level'>" + "<img src='http://img.chemayi.com/public/l" + score + ".png' />" + "</span>" + "<span class='replace_info'>" + "<div class='replace_info_name' spid='" + CMY.PhySpId[classicIndex] + "'>" + CMY.PhyClassicArr[classicIndex] + "</div>" + "<div class='replace_info_sm'>" + CMY.PhyProJy[classicIndex] + "</div>" + "</span>" + "</span>" + "<span class='replace_percent'>" + (CMY.PhyProResult[classicIndex] == 0 ? "" : (CMY.PhyProResult[classicIndex]) + "%") + "</span>" + "<span class='state_now'><b class='level" + score + "'>" + "	<a href='/sp/search/spId-" + CMY.PhySpId[classicIndex] + ".html' target=_blank>" + (score == 1 ? "维持现状" : (score == 4 ? "视情况而定" : (score == 3 ? "必须更换" : "建议检查"))) + "</a>" + "</b></span>" + "</div>";
...
GLOBAL.namespace("Page_User");
GLOBAL.Page_User.mebOredrStep = function() {
    var _curindex;
    var _step_li = $('#meb_order_step li');
    for (var i = 0; i < _step_li.length; i++) {
        if (_step_li.eq(i).hasClass('cur')) {
            _curindex = i
        }
    }
    $('#meb_order_step li').hover(function() {
        if (_step_li.index(this) != _curindex) {
            $(this).addClass('cur')
        }
    }, function() {
        if (_step_li.index(this) != _curindex) {
...
GLOBAL.Page_User.Evaluate = function() {
    var _service_pj_box_xing = $('.service_pj_box_xing');
    _service_pj_box_xing.bind('click', function() {
        var item_parents = $(this).parent().parent('.service_pj_box_con');
        var _curHide = item_parents.find("input[name='curindex']");
        var item_parent = $(this).parent();
        var _cur_item = item_parents.children('.service_pj_box_con_item');
        _cur_item.find('.service_pj_box_xing').removeClass('service_pj_box_xing_on');
        _cur_item.removeClass('service_pj_num_cur');
        _cur_item.find('.service_pj_box_num').html('');
        _cur_item.find('.service_pj_box_info').html('');
        var _index = _cur_item.index(item_parent);
        _curHide.val(_index);
        _cur_item.eq(_index).addClass('service_pj_num_cur');
        quality_numShow(_cur_item, _index);
        for (var i = 0; i < _index + 1; i++) {
            _cur_item.eq(i).find('.service_pj_box_xing').addClass('service_pj_box_xing_on')
        }
    });

    function quality_numShow(_cur_item, _index) {
        switch (_index) {
            case 0:
                _cur_item.eq(_index).find('.service_pj_box_num').html('1分');
                _cur_item.eq(_index).find('.service_pj_box_info').html('不满意');
                break;
            case 1:
                _cur_item.eq(_index).find('.service_pj_box_num').html('2分');
                _cur_item.eq(_index).find('.service_pj_box_info').html('一般');
                break;
            case 2:
                _cur_item.eq(_index).find('.service_pj_box_num').html('3分');
                _cur_item.eq(_index).find('.service_pj_box_info').html('比较满意');
                break;
            case 3:
                _cur_item.eq(_index).find('.service_pj_box_num').html('4分');
                _cur_item.eq(_index).find('.service_pj_box_info').html('满意');
                break;
            case 4:
                _cur_item.eq(_index).find('.service_pj_box_num').html('5分');
                _cur_item.eq(_index).find('.service_pj_box_info').html('非常滿意');
                break
        }
    };
    _service_pj_box_xing.hover(function() {
        var item_parents = $(this).parent().parent('.service_pj_box_con');
        var item_parent = $(this).parent();
        var _cur_item = item_parents.children('.service_pj_box_con_item');
        _cur_item.find('.service_pj_box_xing').removeClass('service_pj_box_xing_on');
        ...
        _cur_item.find('.service_pj_box_info').html('');
        var _curHide = item_parents.find("input[name='curindex']");
        var _curIndex = parseInt(_curHide.val());
        _curIndex = parseInt(_curIndex) + parseInt(1);
        if (_curIndex != -1) {
            for (var i = 0; i < _curIndex; i++) {
                _cur_item.eq(i).find('.service_pj_box_xing').addClass('service_pj_box_xing_on')
            }
            quality_numShow(_cur_item, _curIndex - 1);
            _cur_item.eq(_curIndex - 1).addClass('service_pj_num_cur')
        }
    });

    function quality_evaluate_box(curIndex) {
        var _quality_evaluate_box_item = $('#quality_evaluate .service_pj_box_con_item');
        _quality_evaluate_box_item.removeClass('service_pj_num_cur');
        _quality_evaluate_box_item.find('.service_pj_box_xing').removeClass('service_pj_box_xing_on');
        for (var i = 0; i <= curIndex; i++) {
            _quality_evaluate_box_item.eq(i).addClass('service_pj_num_cur');
            _quality_evaluate_box_item.eq(i).find('.service_pj_box_xing').addClass('service_pj_box_xing_on')
        }
    };
    var quality_evaluate_xing = $('#quality_evaluate .service_pj_box_xing');
    var quality_evaluate_msg = $('#quality_evaluate_msg .service_pj_box_xing');
    quality_evaluate_xing.bind('click', function() {
        document.body.id = 'msgBody'
    });
    quality_evaluate_msg.bind('click', function() {
        document.body.id = '';
        var curIndex = $('#quality_evaluate').find("input[name='curindex']");
        var item_parents = $(this).parent().parent('.service_pj_box_con');
        var item_parent = $(this).parent();
        var _cur_item = item_parents.children('.service_pj_box_con_item');
        var _index = _cur_item.index(item_parent);
        curIndex.val(_index);
        for (var i = 0; i < _index + 1; i++) {
            quality_evaluate_xing.eq(i).addClass('service_pj_box_xing_on')
        }
    })
};
$(function() {
    GLOBAL.Page_User.mebOredrStep()
});