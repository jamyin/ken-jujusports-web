...
(function ($) {
    $.extend($.fn, {
        uploadEx: function (settings) {
        	
        	

            var options = $.extend({
                fileType: "gif|jpg|jpeg|png|bmp",                       //允许的文件格式
                uploadUrl: "/api/uploadEx.do",      //上传URL地址
                ...
                $this.bind("click", function () {
                    ...
                    data.success = function (response, statusText, xhr, $form) {
                        ...
                        if (myObject.status == 200) {
                            ...
                            $("#yin").val(myObject.data.url);
                            //图片回显
                            $("#userImage1").attr("src",myObject.data.url);
                        ...
                        $form.remove();
                        $("#file_licence_ex").html("<input name='file' id='reg_permit' onchange='showImg()' style='opacity:0; filter:alpha(opacity=0); width: 90px;height:50px;' type='file'></input>");
                    };
